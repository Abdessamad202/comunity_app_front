    ...
    <form className="space-y-6" onSubmit={handleSubmit}>
      ...
      <div>
        ...
        <div className="flex gap-4 justify-center">
          {/* Male Option */}
          <label className={`flex items-center gap-2 px-4 py-2 border rounded-lg cursor-pointer transition ${formData.sex === 'M' ? 'bg-indigo-100 border-indigo-500' : 'border-gray-300'}`}>
            <input type="radio" name="sex" value="M" className="hidden" onChange={(e) => handleInputChange(e, setFormData, setErrors)} />
            <div className={`w-5 h-5 flex items-center justify-center border-2 rounded-full transition ${formData.sex === 'M' ? 'border-indigo-500' : 'border-gray-400'}`}>
          ...
          {/* Female Option */}
          <label className={`flex items-center gap-2 px-4 py-2 border rounded-lg cursor-pointer transition ${formData.sex === 'F' ? 'bg-indigo-100 border-indigo-500' : 'border-gray-300'}`}>
            <input type="radio" name="sex" value="F" className="hidden" onChange={(e) => handleInputChange(e, setFormData, setErrors)} />
            <div className={`w-5 h-5 flex items-center justify-center border-2 rounded-full transition ${formData.sex === 'F' ? 'border-indigo-500' : 'border-gray-400'}`}>
